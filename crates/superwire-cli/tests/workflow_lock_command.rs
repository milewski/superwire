...
fn writes_single_project_lock_for_multiple_workflows() {
    ...
    let first_workflow_source = superwire_macros::workflow_source_template! {
        secrets {
            mcp_endpoint: string
        }
    ...
    let second_workflow_source = superwire_macros::workflow_source_template! {
        secrets {
            mcp_endpoint: string
        }
...
fn writes_relative_workflow_keys_when_using_default_output_path() {
    let temporary_workspace = TemporaryWorkspace::new("superwire-workflow-lock-tests");
    let workflow_source = superwire_macros::workflow_source_template! {
        output {
            value: "ok"
        }
    };
...
fn writes_partial_vars_file_even_when_some_workflows_fail_to_parse() {
    let temporary_workspace = TemporaryWorkspace::new("superwire-workflow-lock-tests");
    let valid_workflow_source = superwire_macros::workflow_source_template! {
        input {
            project_id: number
        }
    ...
    let invalid_workflow_source = superwire_macros::workflow_source_template! {
        input {
            project_id:
        }
    };
