use crate::semantic::SingletonDeclarationKind;
use std::collections::{BTreeMap, HashMap};
use superwire_types::ast::{
    Declaration, DeclarationKeyword, ImportKeyword, ReferenceAccess, SourcePosition, SourceSpan, ToolSource, TypeExpression,
    TypeExpressionFieldCache, TypedField, Workflow,
};
...
impl SemanticToolingSnapshot {
    ...
    #[must_use]
    pub fn from_source_tolerant_with_parser<ParseWorkflow>(source_text: &str, parse_workflow: ParseWorkflow) -> Self
    where
        ParseWorkflow: Fn(&str) -> Result<Workflow, Option<SourceSpan>>,
    {
        match parse_workflow(source_text) {
            Ok(workflow) => Self::from_workflow(&workflow),
            Err(parse_error_span) => {
                if let Some(snapshot_from_prefix) = Self::from_prefix_before_parse_error(source_text, parse_error_span, &parse_workflow) {
    ...
    fn from_prefix_before_parse_error<ParseWorkflow>(
        source_text: &str,
        parse_error_span: Option<SourceSpan>,
        parse_workflow: &ParseWorkflow,
    ) -> Option<Self>
    where
        ParseWorkflow: Fn(&str) -> Result<Workflow, Option<SourceSpan>>,
    {
    ...
    pub fn resolve_access_path_types(&self, start_types: Vec<TypeExpression>, access_path_segments: &[String]) -> Vec<TypeExpression> {
        let reference_accesses = access_path_segments.iter().map(ReferenceAccess::required).collect::<Vec<_>>();

        self.resolve_reference_access_path_types(start_types, reference_accesses.as_slice())
    }

    #[must_use]
    pub fn resolve_reference_access_path_types(
        &self,
        start_types: Vec<TypeExpression>,
        reference_accesses: &[ReferenceAccess],
    ) -> Vec<TypeExpression> {
        let mut candidate_types = start_types;
        let mut field_cache = TypeExpressionFieldCache::new();

        for reference_access in reference_accesses {
...
mod tests {
    ...
    fn tolerant_source_snapshot_recovers_symbols_and_singleton_fields_after_parse_failure() {
        ...
        let tooling_snapshot = SemanticToolingSnapshot::from_source_tolerant_with_parser(broken_source, |source_text| {
