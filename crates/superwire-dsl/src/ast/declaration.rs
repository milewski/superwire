...
pub enum ModelAssetKindParseError {
    #[error("model `{model_name}` property `{property_name}` must be an array of string asset kinds")]
    ExpectedArray { model_name: String, property_name: &'static str },

    #[error("model `{model_name}` property `{property_name}` entries must be string literals")]
    ExpectedStringLiteral { model_name: String, property_name: &'static str },

    #[error("model `{model_name}` property `{property_name}` uses unsupported asset kind `{asset_kind_name}`")]
    UnsupportedAssetKind {
        model_name: String,
        property_name: &'static str,
        asset_kind_name: String,
    },
}
...
impl ModelDeclaration {
    ...
    pub fn supported_asset_kinds(&self) -> Result<Vec<ModelAssetKind>, ModelAssetKindParseError> {
        ...
        let Expression::ArrayLiteral(asset_kind_expressions) = &property.value else {
            return Err(ModelAssetKindParseError::ExpectedArray {
                model_name: self.name.clone(),
                property_name: ModelDeclarationPropertyName::Assets.as_str(),
            });
        };
        ...
        for asset_kind_expression in asset_kind_expressions {
            let Expression::StringLiteral(asset_kind_name) = asset_kind_expression else {
                return Err(ModelAssetKindParseError::ExpectedStringLiteral {
                    model_name: self.name.clone(),
                    property_name: ModelDeclarationPropertyName::Assets.as_str(),
                });
            };
            ...
            let Some(asset_kind) = ModelAssetKind::from_identifier(asset_kind_name) else {
                return Err(ModelAssetKindParseError::UnsupportedAssetKind {
                    model_name: self.name.clone(),
                    property_name: ModelDeclarationPropertyName::Assets.as_str(),
                    asset_kind_name: asset_kind_name.clone(),
                });
            };
