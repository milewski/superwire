...
macro_rules! workflow_doc_source {
    ($($workflow_source_tokens:tt)*) => {{
        let workflow_source_template = crate::testing::WorkflowSourceTemplate::from_inline(stringify!($($workflow_source_tokens)*));
        let mut workflow_source = String::new();

        for source_line in workflow_source_template.source().lines() {
            let trimmed_source_line = source_line.trim_start();

            if let Some(workflow_line) = trimmed_source_line.strip_prefix("/// ") {
                workflow_source.push_str(workflow_line);
            } else if let Some(workflow_line) = trimmed_source_line.strip_prefix("///") {
                workflow_source.push_str(workflow_line);
            } else {
                continue;
            }
