...
function installNativeExtension(): void
{
    if ((string) \getenv('ENGINE_AI_FFI_PHP_SKIP_NATIVE_INSTALL') === '1') {
        ...
        echo "Skipping native install because ENGINE_AI_FFI_PHP_SKIP_NATIVE_INSTALL=1\n";
    ...
    if (\is_file($prebuiltBinaryPath)) {
        ...
        echo "Found prebuilt native extension for {$prebuiltPlatformKey}\n";
    ...
    if ($resolvedSourceBinaryPath === '' && \is_file($legacyPrebuiltBinaryPath)) {
        ...
        echo 'Found legacy prebuilt native extension for ' . legacyPlatformKey() . "\n";
    ...
    if ($resolvedSourceBinaryPath === '') {
        ...
        if (!\is_file($localBuiltBinaryPath)) {
            ...
            echo "No prebuilt binary found for {$prebuiltPlatformKey}. Building extension from source...\n";
    ...
    echo "Native extension binary ready at {$installedBinaryPath}\n";

    if ($resolvedIniFilePath !== null) {

        echo "Extension ini written at {$resolvedIniFilePath}\n";
    ...
    if (\getenv('ENGINE_AI_FFI_PHP_INI_DIR') === false) {
        echo "Set ENGINE_AI_FFI_PHP_INI_DIR to auto-write an ini file inside a writable directory.\n";
    }

    echo "Enable it in php.ini with: extension={$installedBinaryPath}\n";
}
...
function resolveInstallMode(): string
{
    ...
    if ($normalizedInstallMode !== ENGINE_AI_FFI_PHP_INSTALL_MODE_LOCAL
        ...
        echo "Unknown ENGINE_AI_FFI_PHP_INSTALL_MODE={$rawInstallMode}. Falling back to local mode.\n";
...
function installBinary(string $sourcePath): string
{
    ...
    if ($installMode === ENGINE_AI_FFI_PHP_INSTALL_MODE_LOCAL) {
        ...
        echo "Using package-local extension binary path. Set ENGINE_AI_FFI_PHP_INSTALL_MODE=system to copy into extension_dir.\n";
...
function copyBinaryToDirectory(string $sourcePath, string $targetDirectory): string
{
    ...
    if (!\is_writable($targetDirectory)) {
        ...
        echo "Extension directory is not writable ({$targetDirectory}); using package-local binary path.\n";
    ...
    if (!@\copy($sourcePath, $targetPath)) {
        ...
        echo "Unable to copy native extension binary to {$targetPath}; using package-local binary path.\n";
