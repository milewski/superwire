    ...
    TypeMarker: JsonSchema,
{
    ...
    let schema_value = serde_json::to_value(schema).map_err(|source| WorkflowRuntimeError::SerializationFailed {
...
fn parse_json_schema(schema_value: &Value, definitions: &HashMap<String, Value>) -> Result<WorkflowType, WorkflowRuntimeError> {
...
fn parse_reference_schema(reference: &str, definitions: &HashMap<String, Value>) -> Result<WorkflowType, WorkflowRuntimeError> {
...
fn parse_union_schema(union_members: &[Value], definitions: &HashMap<String, Value>) -> Result<WorkflowType, WorkflowRuntimeError> {
...
fn parse_type_name(
    ...
    match type_name {
        ...
        _ => Err(WorkflowRuntimeError::Other {
            message: format!("unsupported schema type `{type_name}`"),
        }),
    }
}

fn parse_array_schema(schema_value: &Value, definitions: &HashMap<String, Value>) -> Result<WorkflowType, WorkflowRuntimeError> {
...
fn parse_object_schema(schema_value: &Value, definitions: &HashMap<String, Value>) -> Result<WorkflowType, WorkflowRuntimeError> {
...
pub fn parse_number_literal(number_literal: &str) -> Result<Number, WorkflowRuntimeError> {
