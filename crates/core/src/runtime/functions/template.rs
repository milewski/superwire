...
fn resolve_template_source(template_source_identifier: &str, context: &str) -> Result<String, WorkflowRuntimeError> {
    ...
    if direct_template_path.exists() {
        return fs::read_to_string(direct_template_path).map_err(|error| WorkflowRuntimeError::ExpressionEvaluation {
    ...
    if manifest_relative_template_path.exists() {
        return fs::read_to_string(&manifest_relative_template_path).map_err(|error| WorkflowRuntimeError::ExpressionEvaluation {
            context: context.to_string(),
            message: format!(
    ...
    if manifest_workflows_template_path.exists() {
        return fs::read_to_string(&manifest_workflows_template_path).map_err(|error| WorkflowRuntimeError::ExpressionEvaluation {
            context: context.to_string(),
            message: format!(
                "failed to read template file `{}`: {error}",
    ...
    if workflow_examples_template_path.exists() {
        return fs::read_to_string(&workflow_examples_template_path).map_err(|error| WorkflowRuntimeError::ExpressionEvaluation {
            context: context.to_string(),
            message: format!(
                "failed to read template file `{}`: {error}",
...
mod tests {
    ...
    impl AgentRunner for NoopRunner {
        async fn run_agent(&self, _request: &AgentExecutionRequest) -> Result<AgentExecutionResult, WorkflowRuntimeError> {
            Err(WorkflowRuntimeError::Other {
                message: "template tests should not execute any agents".to_string(),
            })
        }
    }

    #[tokio::test]
    async fn template_function_renders_prompt_file_with_bindings() {
        ...
        let output = runtime
            .run_with_runner(
                Input {
                    study_name: "Q1 Feature Study".to_string(),
                    audience: "Product Leadership".to_string(),
                    findings: vec!["navigation confusion".to_string(), "checkout friction".to_string()],
                },
                &NoopRunner,
            )
            .await
        ...
        assert!(output.rendered_prompt.contains("Study: Q1 Feature Study"));
        assert!(output.rendered_prompt.contains("Audience: Product Leadership"));
        assert!(output
            .rendered_prompt
            .contains("Findings: [\"navigation confusion\",\"checkout friction\"]"));
    }

    #[tokio::test]
    async fn template_function_rejects_missing_bindings() {
        #[derive(Debug, Serialize, JsonSchema)]
        struct Input {
            study_name: String,
            audience: String,
        ...
        assert!(matches!(
            execution_result,
            Err(WorkflowRuntimeError::ExpressionEvaluation { message, .. }) if message.contains("binding `findings` is missing")
        ));
    }
}
