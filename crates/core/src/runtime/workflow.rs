...
pub async fn try_workflow_from_workflow<OutputType>(
    workflow: &Workflow,
) -> Result<Result<OutputType, serde_json::Error>, WorkflowRuntimeError>
where
    OutputType: DeserializeOwned,
{
    try_workflow_from_workflow_with_values(workflow, json!({}), json!({})).await
}

pub async fn try_workflow_from_workflow_with_values<OutputType>(
    workflow: &Workflow,
    input_values: Value,
    secret_values: Value,
) -> Result<Result<OutputType, serde_json::Error>, WorkflowRuntimeError>
...
{
    let runtime = WorkflowRuntime::with_default_providers();
    let execution_result = runtime.execute_workflow(workflow, input_values, secret_values).await?;
