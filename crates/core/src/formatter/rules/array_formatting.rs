...
impl ArrayFormattingRule {
    ...
    pub fn should_break_array(&self, items: &[Value]) -> bool {
        ...
        let total_length: usize = items
            .iter()
            .map(|item| self.estimate_value_length(item))
            .sum();
    ...
    fn estimate_value_length(&self, value: &Value) -> usize {
        match value {
            ...
            Value::Null => 4, // "null"
            Value::Array(items) => {
                let inner: usize = items.iter().map(|item| self.estimate_value_length(item)).sum();
...
mod tests {
    ...
    fn test_should_break_array_long_content() {
        ...
        let long_content_array = vec![
            Value::String("this is a very long string that definitely exceeds the eighty character threshold for breaking arrays".to_string()),
    ...
    fn test_estimate_value_length() {
        ...
        assert_eq!(rule.estimate_value_length(&Value::String("hello".to_string())), 7); // 5 + 2 quotes
        assert_eq!(rule.estimate_value_length(&Value::Boolean(true)), 4);
        assert_eq!(rule.estimate_value_length(&Value::Null), 4);
    }
