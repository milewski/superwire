...
impl ProviderCompletionPropertyName {
    fn from_identifier(identifier: &str) -> Option<Self> {
        match identifier {
            identifier if identifier == Self::Driver.as_str() => Some(Self::Driver),
            identifier if identifier == Self::Models.as_str() => Some(Self::Models),
            _ => None,
        }
    }
...
impl SemanticIndex {
    pub fn model_profile_suggestions(&self, model_prefix: &str) -> Vec<CompletionSuggestion> {
        let mut completion_suggestions = self
            ...
            .map(|(model_name, model_summary)| CompletionSuggestion {
                ...
                insert_text: format!("{}.{model_name}", ReferenceKeyword::Model.as_str()),
            })
            .collect::<Vec<_>>();
    ...
    pub fn provider_driver_value_suggestions(&self, position: Position, line_prefix: &str) -> Option<Vec<CompletionSuggestion>> {
        ...
        if ProviderCompletionPropertyName::from_identifier(property_name_identifier) != Some(ProviderCompletionPropertyName::Driver) {
            return None;
        }
    ...
    pub fn provider_models_value_suggestions(&self, line_prefix: &str) -> Option<Vec<CompletionSuggestion>> {
        ...
        if ProviderCompletionPropertyName::from_identifier(property_name_identifier) != Some(ProviderCompletionPropertyName::Models) {
            return None;
        }
    ...
    pub fn is_type_position(&self, position: Position, line_prefix: &str) -> bool {
        ...
        if trimmed_line_prefix.ends_with(schema_namespace.as_str()) {
            return true;
        }
    ...
    pub fn root_declaration_suggestions(&self, line_prefix: &str) -> Vec<CompletionSuggestion> {
        ...
        let mcp_keyword = DeclarationKeyword::Mcp.as_str();
        let tool_keyword = DeclarationKeyword::Tool.as_str();
        let resource_keyword = DeclarationKeyword::Resource.as_str();
        let prompt_keyword = DeclarationKeyword::Prompt.as_str();
        ...
        if import_keyword.starts_with(declaration_prefix) {
            completion_suggestions.push(CompletionSuggestion {
                label: import_keyword.to_string(),
