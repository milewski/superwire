...
pub fn from_model_string(model: &str) -> Result<(Box<dyn Provider>, String)> {
    ...
    if model == "auto" {
        ...
        let entry = available.iter().find(|e| e.requires_key()).copied().ok_or_else(|| {
...
fn build_provider(entry: &ProviderEntry, model: &str) -> Result<Box<dyn Provider>> {
    match entry.api_format {
        ...
        ApiFormat::OpenAiCompatible => {
            ...
            let provider = OpenAi::builder().base_url(entry.api_base).api_key(key).model(model).build()?;
...
mod tests {
    ...
    fn test_auto_detect_cohere() {
        let (entry, model) = auto_detect("command-r-plus").unwrap_or_else(|_| (registry::lookup("cohere").unwrap(), "command-r-plus"));
