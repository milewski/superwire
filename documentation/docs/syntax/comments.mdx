...
Comment text is not a quoted string: backslash sequences inside `//` and `///` comments are preserved as comment text rather than decoded. The closed escape set applies only inside quoted DSL syntax; see [Quoted-string escapes](/syntax/expressions#quoted-string-escapes).
