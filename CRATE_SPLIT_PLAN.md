...
- Owning item: Create `superwire-provider-cersei` and move the Cersei provider implementation into it. Reason: the Cersei implementation and direct `cersei-provider`/`cersei-types` dependencies now live in `superwire-provider-cersei`, but `superwire-executor` temporarily depends on `superwire-provider-cersei` because executor still owns the default `ExecutorService`, router, serve helpers, and binary that instantiate the Cersei provider. This keeps existing default runtime/server behavior, but `cargo tree -p superwire-executor` still includes Cersei crates transitively through the provider crate. Follow-up: when extracting `superwire-executor-server`, move default Cersei wiring out of `superwire-executor` or require server callers to pass a provider so executor no longer depends on the Cersei provider crate.
