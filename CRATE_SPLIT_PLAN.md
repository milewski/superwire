...
- Owning item: Remove or empty `superwire-core` after all imports are migrated. Reason: `superwire-core` no longer owns MCP modules or the DSL implementation, but it still contains `WorkflowDocument` orchestration and the legacy core testing module used by core's remaining integration tests. Follow-up: move or replace the remaining `WorkflowDocument` callers and delete the testing leftover before emptying `superwire-core`.
