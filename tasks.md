...
- Phase 3 AST split is partially complete: `span.rs`, `keywords.rs`, `reference.rs`, `types.rs`, and `expression.rs` now own source span types, centralized keyword/property enums, reference types, typed-field/type-expression/variant-case behavior, and expression/call/template/object-field behavior with focused tests, with stable re-exports preserved from `dsl::ast` and `dsl::mod`. Remaining AST domains to extract are workflow, declaration, tool, MCP, and agent; `McpImportKind` still lives in `ast.rs`, while `McpCallOperation` moved with expression call types.
